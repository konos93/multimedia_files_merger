...
def concatenate_files(file_paths, output_file, folder):
    # Save absolute file paths and durations to selected_files.txt in the specified folder
    selected_files_path = os.path.join(folder, 'selected_files.txt')
    with open(selected_files_path, 'w', encoding='utf-8') as file:
    ...
    print(f'Selected file paths and durations (with cumulative durations) have been saved to {selected_files_path}')
    ...
    if not os.path.exists(selected_files_path):
        print(f"Error: {selected_files_path} does not exist.")
    else:
...
'''
    ...
    try:
        os.remove(selected_files_path)
    except OSError as e:
        print(f"Error deleting {selected_files_path}: {e}")
...
def main():
    ...
    if not file_paths:
        ...
        # Specify the output file name in the same folder
        output_file = os.path.join(folder, f'output{os.path.splitext(file_paths[0])[1]}')

        # Use ThreadPoolExecutor for concurrent processing
        with ThreadPoolExecutor() as executor:
            executor.submit(concatenate_files, file_paths, output_file, folder)
